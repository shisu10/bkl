...
"""
    ...
    **必传参数 `tokenUrl`**：登录接口地址，用于生成 Swagger UI 授权按钮
    **核心功能**：
    - 从请求头提取 `Authorization: Bearer xxx` 中的 token
    - token 缺失或无效时自动返回 401
    - 配合 `Depends(OAuth2(scopes=["..."]))` 校验 scope 权限

    **可选参数**：
    - `scopes`：定义可用权限（仅 Swagger UI 文档显示，不参与校验）
    - `scheme_name`：文档中显示的名称
    - `auto_error`：是否自动返回 401，默认 `True`
    - `description`：文档描述
"""

        #作用:1.在swagger ui文档中显示scopes
        #    2.配合OAuth2PasswordRequestForm在注入依赖下的scopes检验
        #     检验方式:dependencies=[Security(...)]
                                #从设置中给定token_url
